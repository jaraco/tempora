...
class DatetimeConstructor:
    ...
    def construct_datetime(cls, *args, **kwargs):
        ...
        if len(args) == 1:
            arg = args[0]
            method = cls.__get_dt_constructor(type(arg).__module__, type(arg).__name__)
            result = method(arg)
            try:
            ...
            if kwargs:
                ...
                tmpl = (
                    "{first_key} is an invalid keyword " "argument for this function."
                )
    ...
    def __get_dt_constructor(cls, moduleName, name):
        try:
            ...
            tmpl = "No way to construct datetime.datetime from " "{moduleName}.{name}"
            raise TypeError(tmpl.format(**locals()))
